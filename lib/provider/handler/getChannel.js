...
function parseVideo(video, params) {
  ...
  if (video.ads) {
    ...
    var tag_with_macros = ad_tag.replace('{{deviceWidth}}', deviceWidth).replace('{{deviceHeight}}', deviceHeight).replace('{{app_bundle}}', bundleIdentifier).replace('{{app_name}}', app_name).replace('{{device_devicetype}}', deviceType).replace('{{device_ifa}}', advertisingIdentifier).replace('{{cb}}', Math.round(new Date().getTime() / 1000));
