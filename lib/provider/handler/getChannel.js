...
function parseVideo(video, params) {
  ...
  if (video.ads) {
    ...
    var deviceWidth = params.deviceWidth,
        deviceHeight = params.deviceHeight,
        platform = params.platform,
        deviceType = params.deviceType,
        bundleIdentifier = params.bundleIdentifier,
        advertisingIdentifier = params.advertisingIdentifier,
        app_name = params.app_name;


    var ad_tag = platform === "android" ? android_ad_tag : ios_ad_tag;

    var app_name_encoded = encodeURI(app_name);

    var tag_with_macros = ad_tag.replace('{{deviceWidth}}', deviceWidth).replace('{{deviceHeight}}', deviceHeight).replace('{{app_bundle}}', bundleIdentifier).replace('{{app_name}}', app_name_encoded).replace('{{device_devicetype}}', deviceType).replace('{{device_ifa}}', advertisingIdentifier).replace('{{cb}}', Math.round(new Date().getTime() / 1000));
